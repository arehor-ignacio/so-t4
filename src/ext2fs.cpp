...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	// Indirect Simple Case 
	if (block_number <= SINGLE_INDIRECT_BLOCK_FINISH){
		//Read from indirect simple block
		
		//Create a buffer and search the block in second memory
		unsigned char buffer[block_size];
		unsigned int indirect_table_addr = inode->block[SINGLE_BLOCK_INDEX];
		
		read_block(indirect_table_addr,buffer);

		return buffer[(block_number - SINGLE_INDIRECT_BLOCK_START) * sizeof(unsigned int)];
	
	}
	if (block_number <= DOUBLE_INDIRECT_BLOCK_FINISH){
		// Indirect Double Case

		//Create two buffers, one for each table
		unsigned char buffer_single[block_size];
		unsigned char buffer_double[block_size];
		unsigned int indirect_table_addr = inode->block[DOUBLE_BLOCK_INDEX];
		
		//Calcule the index of each block. 
		unsigned int single_block_index = (block_number - DOUBLE_INDIRECT_BLOCK_START) / BLOCK_ENTRIES;
		unsigned int double_block_index = (block_number - DOUBLE_INDIRECT_BLOCK_START) % BLOCK_ENTRIES;
		

        // Search the blocks in second memory.
		read_block(indirect_table_addr,buffer_single);
		read_block(buffer_single[single_block_index * sizeof(unsigned int)] ,buffer_double);
        
        return buffer_double[double_block_index * sizeof(unsigned int)];
	}
	else {
		// Indirect Triple Case
		
		//Create three buffers, one for each table
		unsigned char buffer_single[block_size];
		unsigned char buffer_double[block_size];
		unsigned char buffer_triple[block_size];
		unsigned int indirect_table_addr = inode->block[TRIPLE_BLOCK_INDEX];
		
		//Calcule the index of each block. 
		unsigned int single_block_index = (block_number - TRIPLE_INDIRECT_BLOCK_START) / BLOCK_ENTRIES * BLOCK_ENTRIES ;
		unsigned int double_block_index = ((block_number - TRIPLE_INDIRECT_BLOCK_START) % (BLOCK_ENTRIES * BLOCK_ENTRIES)) / BLOCK_ENTRIES ;
		unsigned int triple_block_index = (block_number - TRIPLE_INDIRECT_BLOCK_START) % BLOCK_ENTRIES;
		
		// Search the blocks in second memory.
		read_block(indirect_table_addr,buffer_single);
		read_block(buffer_single[single_block_index * sizeof(unsigned int)] ,buffer_double);
		read_block(buffer_double[double_block_index * sizeof(unsigned int)] ,buffer_triple);
		
        
		return buffer_triple[triple_block_index];
	}
